...
which repository the user means, stop and ask. This matters most for `cypher`, whose
...
**query** finds code related to the error.
...
**context** gives full context for a suspect.
...
**cypher** provides custom call chain traces. Pass `repo` alongside the statement.
The Cypher text itself names no repository, so the result is unattributable without
it.
...
**trace** finds the shortest call chain between two symbols ("how does A reach B?"). Use one call instead of chaining `context` hops.
...
When no path exists, `trace` reports the furthest reachable node. This is exactly where the chain breaks (dynamic dispatch, reflection, or an external boundary).
