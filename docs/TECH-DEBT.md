...
|---|---|---|---|---|---|---|---|
| Static OAuth fallback reports `272000` as a context limit. | `src/data/openai-oauth-models.ts` | Behavior | Incorrect Claude Code context display and limit handling. | Medium | Codex | P0 | Open |
...
| Pre-compaction stream break has no captured request or response trace. | Claude Code to provider stream | Evidence | Root cause remains unproven. | Small | Codex | P0 | Open |
...
| Hidden context provenance. | OAuth seed and resolver paths. | Introduce source-aware metadata. | Codex | P0 | Pending |
