...
**Scope confirmed runtime-reachable, no legacy V1 patch runtime remaining beyond the documented reader.** `src/patcher.ts` is 215 lines: `runPatchCommand`/`runLaunchPatchCheck` delegate exclusively to the V2 transaction path (`runPatchCommandV2`/`runLaunchPatchCheckV2`/`diagnosePatchV2`, all in `src/patch-*.ts`); the sole remaining V1 surface is `readPatchManifest`, owned by `src/patch-legacy-recovery.ts` for verified migration/recovery planning and passed into `src/patch-diagnostics.ts` for read-only reporting — exactly the narrowly scoped migration reader the plan calls for, with no duplicate apply/write path. Every new Phase C–F module (`atomic-file`, `checkpoint-store`, `claude-installation`, `deadline-manager`, `durable-io`, `execution-checkpoint`, `execution-query`, `execution-recovery`, `execution-session-key`, `execution-tracking`, `executions-command`, `http-proxy/routing-decision`, `listener-ready`, `patch-classify`, `patch-diagnostics`, `patch-injection`, `patch-legacy-recovery`, `patch-lock`, `patch-presenter`, `patch-reconcile`, `patch-state`, `patch-transaction`, `provider-capabilities`, `provider-error`, `provider-runtime-cache`, `reconcile-tool-call-workflow`, `registry/credential-cleanup-journal`, `registry/credential-lifecycle`, `registry/lock`, `request-execution-context`, `request-lifecycle-error-mapping`, `request-lifecycle`, `tool-call-ledger`, `tool-call-tap`) was confirmed imported from at least one other production `src/` file (not only its own test), i.e. wired into a live entry point rather than orphaned.
...
## 25. Verified legacy-baseline recovery closure

A live macOS installation exposed a V1→V2 dead-end after the legacy manifest's recorded patched hash diverged from an otherwise functional injected Claude binary. The read-only diagnosis found a valid pristine legacy backup, an empty per-target V2 directory, and no pending transaction. Launch offered to patch, exact-hash migration correctly refused, and the V2 transaction then correctly refused to patch on top of unowned injected bytes; because no recovery baseline was threaded into that transaction, every launch repeated the same prompt.

The fix keeps the fail-closed boundary while making the verified path actionable:

- `src/patch-legacy-recovery.ts` independently verifies canonical target ownership, live version/injection state, backup version, absence of injection markers, and the exact recorded pristine SHA-256. Exact legacy/live matches may be adopted; divergent injected targets expose only a `legacy-migrated` pristine recovery baseline.
- `src/patch-transaction.ts` accepts that baseline only for a missing-manifest recovery, revalidates it immediately before writing to close the inspection/write race, patches a same-directory stage built from pristine bytes, and publishes normal journaled V2 state. It never patches the unknown injected live bytes.
