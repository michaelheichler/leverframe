...
describe('fetchTemplateModels', () => {
  ...
  it('redacts an opaque resolved credential echoed by an upstream error', async () => {
    const root = mkdtempSync(join(tmpdir(), 'leverframe-trace-secret-'));
    const previousHome = process.env['LEVERFRAME_HOME'];
    const previousTrace = process.env['LEVERFRAME_TRACE'];
    const secret = 'opaque.+credential[42]';
    process.env['LEVERFRAME_HOME'] = root;
    process.env['LEVERFRAME_TRACE'] = '1';
    vi.mocked(fetch).mockResolvedValue({
      ok: false,
      status: 500,
      text: async () => `provider echoed ${secret} twice: ${secret}`,
    } as Response);
    try {
      ...
      if (previousTrace === undefined) delete process.env['LEVERFRAME_TRACE'];
      else process.env['LEVERFRAME_TRACE'] = previousTrace;
