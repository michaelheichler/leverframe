...
import { getInferenceSessionLogPath } from '../src/log-paths.js';
import {
...
describe('trace log redaction', () => {
  ...
  it('redacts every occurrence of an opaque resolved secret literally', () => {
    const secret = 'opaque.+credential[42]';
    const line = `first=${secret} second=${secret}`;
    expect(redactTraceLine(line, [secret])).toBe(
      'first=[REDACTED] second=[REDACTED]',
    );
  });

  it('does not register dangerously short literal values', () => {
    expect(redactTraceLine('status=local', ['local'])).toBe('status=local');
  });
