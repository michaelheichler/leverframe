...
describe('registry/add-template', () => {
  ...
  it('fails if npm is not available', async () => {
    ...
    expect(res.added).toBe(false);
    expect(res.error).toContain('is not available in leverframe');
  });
  ...
  it('reports keyVerified=true for a template that probes the upstream API', async () => {
    const res = await addProviderFromTemplate(dummyTemplate, 'key_123');
    expect(res.added).toBe(true);
    expect(res.keyVerified).toBe(true);
    expect(res.hint).toBeUndefined();
  });

  it('reports keyVerified=false and a truthful hint when skipKeyVerification is set', async () => {
    const seedTemplate: ProviderTemplate = {
      ...dummyTemplate,
      id: 'zai-parity',
      name: 'z.ai (Coding Plan)',
      modelSource: 'static-seed',
      skipKeyVerification: true,
      staticModels: [{ id: 'glm-5.2', name: 'GLM-5.2', contextWindow: 1_000_000 }],
    };
    const res = await addProviderFromTemplate(seedTemplate, 'key_123');
    expect(res.added).toBe(true);
    expect(res.keyVerified).toBe(false);
    expect(res.hint).toContain('did not verify');
    expect(res.hint).not.toContain('test');
  });
});
