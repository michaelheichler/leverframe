...
describe('server model catalog', () => {
  ...
  it('resolves canonical leverframe ids and saved short aliases without advertising them', () => {
    ...
    expect(catalog.get('leverframe:go:deepseek-test')).toMatchObject({ id: 'deepseek-test' });
    expect(catalog.get('leverframe:openai:gpt-5')).toMatchObject({ id: 'gpt-5' });
