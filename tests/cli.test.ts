...
describe('parseArgs', () => {
  ...
  it('parses patch --diagnose, --json, and --target', () => {
    expect(parseArgs(['patch', '--diagnose'])).toMatchObject({ command: 'patch', patchDiagnose: true });
    expect(parseArgs(['patch', '--diagnose', '--json'])).toMatchObject({
      command: 'patch',
      patchDiagnose: true,
      patchJson: true,
    });
    expect(parseArgs(['patch', '--target', '/opt/claude/claude'])).toMatchObject({
      command: 'patch',
      patchTarget: '/opt/claude/claude',
    });
    expect(parseArgs(['patch', '--target=/opt/claude/claude'])).toMatchObject({
      command: 'patch',
      patchTarget: '/opt/claude/claude',
    });
    expect(parseArgs(['patch', '--target'])).toMatchObject({ error: 'Missing value for --target' });
  });

  it('rejects --json without --diagnose', () => {
    expect(parseArgs(['patch', '--json']).error).toMatch(/--json only applies to/);
  });
...
describe('main dispatch', () => {
  ...
  it('patch --diagnose --json is read-only and prints ANSI-free JSON, honoring --target', async () => {
