...
vi.mock('../src/config.js', () => ({
  getSavedServerPassword: () =>
    state.savedPassword === null
      ...
      : { status: 'ok' as const, password: state.savedPassword },
  ...
  setSavedServerPassword: async (password: string) => {
...
// ~/.leverframe/server-runtime.json (a live server may be advertised there).
...
describe('runServerCommand', () => {
  beforeEach(() => {
    ...
    delete process.env['LEVERFRAME_NO_DISCOVERY'];
  });
  ...
  it('starts local mode on 127.0.0.1 with a per-start gateway token', async () => {
    ...
    const password = (state.startServerOptions as { serverPassword: string | null }).serverPassword;
    expect(typeof password).toBe('string');
    expect(password && password.length).toBeGreaterThanOrEqual(40);
  ...
  it('LEVERFRAME_NO_DISCOVERY=1 also skips discovery registration', async () => {
    process.env['LEVERFRAME_NO_DISCOVERY'] = '1';
    const { runServerCommand } = await import('../src/server/index.js');
    const result = runServerCommand();
    await vi.waitFor(() => expect(state.startServerOptions).not.toBeNull());
    process.emit('SIGINT');
  ...
  it('quick starts from saved settings without prompting for start mode or listen mode', async () => {
    ...
    // Local quick-start mints a per-start token even with no user password.
    const password = (state.startServerOptions as { serverPassword: string | null }).serverPassword;
    expect(typeof password).toBe('string');
    expect(password && password.length).toBeGreaterThanOrEqual(40);
  });
  ...
  it('reports EADDRINUSE with an actionable message instead of an Unexpected error', async () => {
    const { startServer } = await import('../src/server/router.js');
    const error = Object.assign(new Error('listen EADDRINUSE'), { code: 'EADDRINUSE' });
    vi.mocked(startServer).mockRejectedValueOnce(error);
    const chunks: string[] = [];
    ...
    const { runServerCommand } = await import('../src/server/index.js');
    const result = await runServerCommand({ quick: true, port: 17645 } as any);

    expect(result).toBe(1);
    const message = chunks.join('');
    expect(message).toMatch(/Port 17645 is already in use/);
    expect(message).toMatch(/--port/);
    writeSpy.mockRestore();
  });

  it('reports EACCES with a privileged-port hint', async () => {
    const { startServer } = await import('../src/server/router.js');
    ...
    const { runServerCommand } = await import('../src/server/index.js');
    const result = await runServerCommand({ quick: true, port: 80 } as any);

    expect(result).toBe(1);
    const message = chunks.join('');
    expect(message).toMatch(/Permission denied/);
    expect(message).toMatch(/1024 or higher/);
    writeSpy.mockRestore();
  });
});
