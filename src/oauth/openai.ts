...
import { withAbortTimeout, OAUTH_REQUEST_TIMEOUT_MS } from './fetch-timeout.js';
...
export async function pollOpenAiDeviceCodeToken(
  ...
  while (now() < deadline) {
    ...
    let response: { status: number; data?: { authorization_code: string; code_verifier: string } };
    try {
      response = await withAbortTimeout(async signal => {
        ...
        return {
          ...
          data: result.ok
            ? await result.json() as { authorization_code: string; code_verifier: string }
            : undefined,
        };
    ...
    if (response.data) {
      ...
      try {
        tokens = await withAbortTimeout(async signal => {
          ...
          return tokenResponse.json() as Promise<OAuthTokenResponse>;
